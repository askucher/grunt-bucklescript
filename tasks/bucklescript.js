...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('bucklescript', 'Compile Bucklescript files', function() {
    var dest = this.file.dest,
        options = this.data.options,
        extension = this.data.extension;

    grunt.file.expandFiles(this.file.src).forEach(function(filepath) {
      grunt.helper('bucklescript', filepath, dest, grunt.utils._.clone(options), extension);
    });
  ...
  grunt.registerHelper('bucklescript', function(src, destPath, options, extension) {
    ...
    var dest = path.join(destPath, path.basename(src, '.ocaml') + extension);
    ...
    try {
      js = bucklescript.compile(grunt.file.read(src), options);
